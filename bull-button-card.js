...
// ─────────────────────────────────────────────
// ICON ANIMATION MAP
// Returns a CSS animation string for a given mdi: icon name
// when the entity is ON, or “” if no animation applies.
// ─────────────────────────────────────────────
function getBullIconAnimation(icon) {
if (!icon) return “”;
const n = icon.toLowerCase();

// Continuous spin — fans, rotors, loading spinners, refresh
if (/fan|turbine|rotate|spinner|loading|sync|refresh|reload|propeller|rotor|windmill|wind-turbine|ceiling-fan/.test(n))
return “bull-spin 1.5s linear infinite”;

// Heartbeat / pulse — heart, health, medical
if (/heart|pulse|cardio|health|ecg/.test(n))
return “bull-heartbeat 1.2s ease-in-out infinite”;

// Ring / shake — bells, alerts, alarms, notifications
if (/bell|alarm|alert|siren|doorbell|ring|notification/.test(n))
return “bull-ring 1s ease-in-out infinite”;

// Bounce — motion, running, steps
if (/run|walk|motion|step|jump|bounce/.test(n))
return “bull-bounce 0.8s ease-in-out infinite”;

// Sway — water, wave, washing
if (/water|wave|wash|ripple|swim/.test(n))
return “bull-sway 2s ease-in-out infinite”;

// Flash scale — power, lightning, energy, electric
if (/power|lightning|flash|electric|energy|bolt|zap/.test(n))
...
if (!pill) return;
const on   = this._isOn();
const icon = this.shadowRoot.querySelector(“ha-icon.bull-icon”);
...
```
// Resolve font-weight and font-style from the font_style option
const fontStyleMap = {
...
const fs = fontStyleMap[c.font_style] || fontStyleMap["normal"];

// Icon animation — only applied when entity is ON via .icon-active class
const iconAnim = getBullIconAnimation(c.icon || "");
...
this.shadowRoot.innerHTML = `
  <style>
    ...
    ${iconAnim ? `ha-icon.bull-icon.icon-active { animation: ${iconAnim}; }` : ""}
    .bull-name {
